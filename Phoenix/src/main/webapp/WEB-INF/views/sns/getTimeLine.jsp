...
<body>
    ...
    ********************-->
	...
	<!-- ToolBar Start /////////////////////////////////////-->
	<jsp:include page="../sns/snsToolbar.jsp" />
	<!-- ToolBar End /////////////////////////////////////-->
	...
	<div class="container-fluid">
		<div class="row">
			...
			<div class="col-lg-8 col-xl-12">
					...
					<div class="card-body">
						<form class="form-profile">
							<div class="form-group">
								<textarea class="form-control" name="textarea" id="post"
				...
				<input type="hidden" id="faceSize"
					value="${faceSearch.currentPage }" /> <input type="hidden"
					id="instaSize" value="${instaSearch.pageSize}" /> <input
					type="hidden" id="total" value="${instaSearch.status}" />
							...
							<div class="media media-reply">
								...
								<div class="media-body">
									<div class="d-sm-flex justify-content-between mb-2">
										<h5 class="mb-sm-0">
											${timeLine.postId} <small class="text-muted ml-3">${timeLine.regDate }</small>
										</h5>
									...
									<p>${timeLine.post }</p>
									<input type="hidden" class="postSize"
										value="${timeLine.postSize }" />

									<c:set var="j" value="0" />

									<c:if test="${!empty timeLine.img1List}">
										<!-- 이미지1 -->
										<c:forEach var="img1List" items="${timeLine.img1List }"
											varStatus="status">
											<c:set var="j" value="${j+1}" />
											<a href="${timeLine.img1LinkList[status.index] }"><img
												alt="" src="${img1List}"
									...
									<c:if test="${!empty timeLine.img2List}">
										<!-- 이미지2 -->
										<c:forEach var="img2List" items="${timeLine.img2List }"
											varStatus="status">
											<c:set var="j" value="${j+1}" />
											<a href="${timeLine.img2LinkList[status.index] }"><img
												alt="" src="${img2List}"
												style="margin-left: 20px; margin-top: 10px"></a>
										</c:forEach>
									</c:if>
									...
									<c:if test="${!empty timeLine.img3List}">
										<!-- 이미지3 -->
										<c:forEach var="img3List" items="${timeLine.img3List }"
											varStatus="status">
											<c:set var="j" value="${j+1}" />
											<a href="${timeLine.img3LinkList[status.index] }"><img
												alt="" src="${img3List}"
												style="margin-left: 20px; margin-top: 10px"></a>
										</c:forEach>
									</c:if>

									<c:if test="${!empty timeLine.img4List}">
										<!-- 이미지4 -->
										<c:forEach var="img4List" items="${timeLine.img4List }"
											varStatus="status">
											<c:set var="j" value="${j+1}" />
											<a href="${timeLine.img4LinkList[status.index] }"><img
												alt="" src="${img4List}"
												style="margin-left: 20px; margin-top: 10px"></a>
										</c:forEach>
									</c:if>

									<c:if test="${!empty timeLine.videoList}">
										<!-- 동영상 -->
										<c:forEach var="videoList" items="${timeLine.videoList }"
											varStatus="status">
											<c:set var="j" value="${j+1}" />
											<a href="${timeLine.videoLinkList[status.index] }"><img
												alt="" src="${videoList }"
												style="margin-left: 20px; margin-top: 10px"></a>
										</c:forEach>
							...
							<div class="media media-reply">
								...
								<div class="media-body">
									...
									<c:if test="${!empty timeLine.videoList}">
										...
										<c:forEach var="videoList" items="${timeLine.videoList }">
											<c:set var="j" value="${j+1}" />
											<video src="${videoList }" autoplay="autoplay"
												controls="controls" width="300px"
												style='margin-left: 20px; margin-top: 10px'></video>
										</c:forEach>
									</c:if>

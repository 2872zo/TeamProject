...
public class SnsDaoImpl implements SnsDao {
	...
	public Account fbLogIn(Account account) {
			...
			System.out.println("account 확인"+account);
			...
			wait = new WebDriverWait(driverTest, 20);
			...
			// 로그인
			webElement = driverTest.findElement(By.id("email"));
			webElement.sendKeys(account.getAccountId());

			webElement = driverTest.findElement(By.id("pass"));
			webElement.sendKeys(account.getAccountPw());

			webElement = driverTest.findElement(By.id("loginbutton"));
			webElement.submit();

			// 로그인 성공인지 실패인지 찾기
			// WebElement login = driver.findElement(By.id("login_link"));
			WebElement feed = driverTest.findElement(By.cssSelector("div[role='feed']"));

			if (feed.isEnabled()) {
				System.out.println("페이스북 로그인 성공");
				account.setAccountDomain("faceBook");
				driverTest.quit();
				return addSnsAccount(account);
			...
			int count=0;
				...
				for (int i = value; i < check.size(); i++) {
					...
					if(post.size()!=0) {//포스트가 없는경우도 존재
						wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("[data-testid='post_message']")));
					...
					wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("span[class='fsm fwn fcg']")));
			...
			for (int i = search.getStatus(); i < article.size(); i++) {
				...
				if(common.size()!=0) {
					...
					for(int j = 0;j<(common.size()-1);j++) {//사진개수-1를 다 가져오기 위해 버튼 클릭 용
						...
						WebElement middle =	article.get(i).findElement(By.cssSelector("div[class='_97aPb ']"));	
